---
title: 'N7: Wrecked Merchant Freighter'
url: https://masseffect.fandom.com/wiki/N7:_Wrecked_Merchant_Freighter
game: Mass Effect 2
type: mission
characters: []
scraped: '2026-08-28'
...
**Difficulty:** While this mission is technically playable as soon as you gain access to the Normandy, it can be fairly challenging to complete on a low\-level character with limited powers, weapons, upgrades, squadmates, and other resources, especially on higher difficulties.

You will want to bring a squad with Incinerate or other anti\-armor powers if undertaking this mission on Hardcore or Insanity. On all difficulties, anti\-mech abilities such as Disruptor Ammo, Overload and especially a high level AI Hacking will be useful. A squad member armed with a sniper rifle can also be an asset.

If you want heavy weapon support, the M\-100 Grenade Launcher or the M\-622 Avalanche are the recommended choices, though the ML\-77 Missile Launcher's homing and indirect fire capabilities can make it a viable option as well.
...
To start searching, turn around to find a crate of 560 platinum. Turn back around, and off to the left, near the second ramp, are some damaged mech parts that you can salvage for 1,500 credits. In the middle of the opening area where the shuttle landed is a datapad with the crashed ship's System Status Report. The report indicates that the ship's VI went out of control and the Captain locked it out. Before proceeding closer to the crash site, there is another set of damaged mech parts that are salvageable to the right worth 1,500 credits.
...
1. ***Destroy Every Mech***: Given that the cargo manifest lists 180 LOKI's and a YMIR, the brute force method will of necessity take a while, but can bring that certain sense of a job well done. Destroying all mechs will automatically end the mission, so if you're planning on doing this approach, make sure you gathered all loot before you kill the last mech.
2. ***The Sneaky Way***: An Infiltrator with the Enhanced Cloak power can complete this assignment without firing a single bullet, simply by running back to the shuttle cloaked (ducking for cover for the couple of seconds it takes the power to regenerate).
3. ***Advance Methodically***: By advancing methodically in between waves of mechs you can gradually work your way toward the shuttle with a reasonable degree of safety. When a wave of mechs is eliminated move up and closer to the shuttle, then get back into cover to deal with the next wave. It's easier to move up if you wait until only one mech remains in a wave. Use pull, singularity, or stasis on on the mech to slow, but not kill the mech. While your squad mates finish him off, you can advance to the next cover position.
4. ***Brute Force*:** A Vanguard can Charge to sprint from enemy to enemy and be in the shuttle in three sprints. End up at the LOKI mechs behind the shuttle and it's possible to jump right in.
...
| --- | --- | --- |
|  | Assignments | N7: Abandoned Research Station |
