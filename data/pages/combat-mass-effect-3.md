---
title: Combat (Mass Effect 3)
url: https://masseffect.fandom.com/wiki/Combat_(Mass_Effect_3)
game: Mass Effect 3
type: lore
characters: []
scraped: '2026-08-28'
...
Combat abilities usable by player characters and NPC allies in Mass Effect 3 are known as powers. Powers can be both active, requiring manual or deliberate usage, or they can be passive, providing constant bonuses as long as squad points have been invested in them. Mass Effect 3 reintroduces grenades as a type of power that is exempt from the universal recharge time that almost all other active powers are subject to, but with the drawback of being limited in quantity. Power Combos are a unique new mechanic in Mass Effect 3 where two powers can interact to create large, damaging explosions.
...
Headshot damage can be further augmented, such as by certain ranks of the various class powers, or certain evolutions of ammo powers and various armor types in single\-player. Headshot damage bonuses from the Pistol Cranial Trauma System weapon mod, ammo powers, and the Marksman power are multiplicative, while all other bonuses are additive, such as from the Vulnerability VI gear and Targeting VI equipment in multiplayer. The multiplicative bonuses **do not** work against the above listed boss\-type enemies, but they provide a stronger damage increase against all lesser enemies. In practice, expect hits against the heads/weak points of boss enemies to deal about 40% extra damage unless additive bonuses are equipped.

Weapons that fire explosive projectiles such as the Acolyte cannot score headshots.
...
- In multiplayer, the player can utilize the temporary invulnerability granted by an Ops Pack to survive the sync kills of the Brute and Phantom.
...
Shield gate is a mechanic introduced in Mass Effect 3\. When an enemy's shields or barriers are destroyed by a non\-melee weapon hit, excess damage from the shot that passes through to health or armor is reduced by a certain percentage according to the difficulty. The reductions are as follows:
...
Weapons that do not fire standard bullets, including any that fire explosive projectiles, ignore the enemy shield gate effect, except for the Krysae Sniper Rifle in multiplayer. Powers and Power Combos also ignore the gate.
...
There are a few types of attacks that completely bypass all gate effects and can instantly kill the player from full health/barrier/shield:
