---
title: Normandy
url: https://masseffect.fandom.com/wiki/Normandy
game: Mass Effect
type: lore
characters: []
scraped: '2026-08-28'
---

|  | **This article is a disambiguation page.** The following is a list of links to pages that might share similar titles or cover similar topics. Please follow one of the disambiguation links below or search to find the page you were looking for if it is not listed. If an internal link led you here, you may wish to change the link to point directly to the intended article. |
| --- | --- |

Normandy may refer to:

- Ships:
	- The SSV Normandy (also known as the Normandy SR\-1\), the experimental stealth frigate from Mass Effect.
	- The Normandy SR\-2, a larger frigate derivative in design to the previous ship of this name in Mass Effect 2 and Mass Effect 3.

- Mass Effect 2 Missions:
	- Normandy: FBA Couplings
	- Normandy: Serrice Ice Brandy
	- Normandy: Special Ingredients
	- Normandy Crash Site (assignment) \- a DLC mission.

- Other Uses:
	- Normandy Crash Site \- a DLC pack from Mass Effect 2\.
